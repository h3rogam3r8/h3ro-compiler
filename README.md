...
Each phase should end with something I can actually run. The boxes are
generated by `scripts/roadmap.py` from the test results, so a box only
gets ticked when there are green tests behind it.
...
- [ ] **6. Down to loops**
  ...
  - [ ] Bufferization
  - [ ] Memory planning by graph coloring
- [ ] **7. CPU backend**
  - [ ] Vectorization
  - [ ] JIT through LLVM
